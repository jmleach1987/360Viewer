...
class DomeScript {
	constructor( object ) {
		...
		*/
		...
		let camera, scene, renderer, raycaster, ImageDome, controls, effect, floor, measurecollide;
		//let plight, dirLight;
		...
		function Init() {
			...
			//
			let geometry = new THREE.SphereGeometry( 50010, 60, 40 ); // radius, division height, division width
			let DomeMaterial = new THREE.MeshBasicMaterial({
				side: THREE.BackSide,
				transparent: true,
				opacity: 1,
			});
			ImageDome = new THREE.Mesh( geometry, DomeMaterial );
			...
			/*
			let DomeMaterial2 = new THREE.MeshBasicMaterial({
				side: THREE.BackSide,
				transparent: true,
				opacity: 1,
			});
			ImageDome2 = new THREE.Mesh( geometry, DomeMaterial2 );
			scene.add(ImageDome2);
			ImageDome2.scale.set(0.5, 0.5, 0.5);
			ImageDome2.material.opacity = 1;
			*/
		...
		*/
		...
		function PositionRig() {
			...
			if (SceneData.flip != undefined) {
				...
				if (SceneData.CamPos[chosenPOV].ry == undefined){
					...
					ImageDome.rotation.y = THREE.Math.degToRad( SceneData.CamPos[chosenPOV].ry+90 );
				}
			}

			let domeImage = SceneData.CamPos[chosenPOV].name;
			ImageDome.material.map = DomeTextures[domeImage];
			ImageDome.visible = true;
		...
		function DrawSparkSprite(pos) {
			...
			if (pos.y > 70) spriteMap = sparkSpriteMapAerial;
		...
		function DrawOtherPOVs(){
			...
			for (let i=0; i < SceneData.CamPos.length; i++){
				...
				let sprite = DrawSparkSprite(pos);
				sprite.name = cm.name;
		...
		function WriteAllData(object) {
			...
			keys.forEach(function(entry) {
				if (!object[entry] == "") {
					if (entry.indexOf("_link") > -1) { // links
						write += entry.replace("_link", "") + ": <a target=\"_blank\"  href=\"" + object[entry] +  "\">PDF link</a></br>" ;
					}
					else { write += "<div class=\"objInfo_Block\"><p><span class=\"objInfo_Label\">" + entry + ":</span><span class=\"objInfo_Value\">" + object[entry] +  "</span></p></div></br>" ; }
				}
