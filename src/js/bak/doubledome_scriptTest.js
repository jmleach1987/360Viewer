...
var camera, scene, renderer, raycaster, plight, ImageDomeA, ImageDomeB, VisibleDome, NotVisibleDome, dirLight, controls, effect;
...
if (a.indexOf("?") > 0) {
	...
	script.src='src/Dome_Projects/'+wd.project+'/CamPos.js';
...
function init() {
	...
	var DomeMaterialA = new THREE.MeshBasicMaterial({
		side: THREE.BackSide,
		transparent: true,
		opacity: 1,
	});
	ImageDomeA = new THREE.Mesh( geometry, DomeMaterialA );
	//
	var DomeMaterialB = new THREE.MeshBasicMaterial({
		side: THREE.BackSide,
		transparent: true,
		opacity: 0.000001,
	});
	geometry = new THREE.SphereGeometry( 50000, 60, 40 ); // radius, division height, division width
	ImageDomeB = new THREE.Mesh( geometry, DomeMaterialB );
	
	scene.add(ImageDomeA);
	scene.add(ImageDomeB);
	//
	VisibleDome = ImageDomeA;
	NotVisibleDome = ImageDomeB;
	// ###############
...
function preloadImages(){
	...
	//	var img = new Image();
	//	var src = 'src/Dome_Projects/'+wd.project+'/' + lowRes + wd.CamPos[i].name+'.jpg';
	//	$(img)
...
function HideShowImage(event) {
	...
	/*
	if (VisibleDome.visible) {
		$("#HideShowImageButton")[0].innerText = "Show Image";
	} else {
		$("#HideShowImageButton")[0].innerText = "Hide Image";
	}
	*/
	if (VisibleDome.material.opacity < 1) {
		VisibleDome.material.opacity = 1;
	} else {
		VisibleDome.material.opacity = 0.46;
	}
	//VisibleDome.visible = !VisibleDome.visible;
}
...
function PositionRig() {
	...
	if (VisibleDome == ImageDomeA) {
		VisibleDome = ImageDomeB;
		NotVisibleDome = ImageDomeA;
	} else {
		VisibleDome = ImageDomeA;
		NotVisibleDome = ImageDomeB;
	}

	VisibleDome.position.set( pos.x, pos.y, pos.z );
	// Rotates the sphere to be at the correct world orientation
	VisibleDome.rotation.y = THREE.Math.degToRad( wd.CamPos[chosenPOV].ry+90 );

	var domeImage = wd.CamPos[chosenPOV].name;
	VisibleDome.material.map = DomeTextures[domeImage];
	console.log(VisibleDome);
	//VisibleDome.material.needsUpdate = true;
	
	//if (ImageDome.material.envMap != undefined) {
	//	ImageDome.material.envMap = ImageDome.material.map;
	//}
	//THREE.ImageUtils.loadTexture( 'src/Dome_Projects/'+wd.project+'/' + lowRes + wd.CamPos[chosenPOV].name +'.jpg' );
	//
...
function GetAllPOVAngles() {
	...
	for (var i=0; i < wd.CamPos.length; i++){
		...
		var vector = new THREE.Vector3( wd.CamPos[i].x, wd.CamPos[i].y, wd.CamPos[i].z );
...
function drawSelectionArc() {
	...
	var seconds = 3;
	tm += (difftime/(seconds*1000)) * (Math.PI*4);
	...
	for (i=0; i<circles.length; i++) {
		...
		var ctx = c.getContext("2d");
		ctx.clearRect(0, 0, c.width, c.height);
		ctx.lineWidth = 0.2 + (1.3*tm);
		ctx.strokeStyle = "rgba(255,255,255,0.7)";
		ctx.beginPath();
		ctx.arc(50, 50, 40, false, tm);
		ctx.stroke();
	}
}
...
function update() {
	...
	*/
	...
	if (VisibleDome.material.opacity < 1) VisibleDome.material.opacity += 0.01;
	if (VisibleDome.material.opacity > 1) VisibleDome.material.opacity = 1;
	if (NotVisibleDome.material.opacity > 0.000001) NotVisibleDome.material.opacity -= 0.01;
	if (NotVisibleDome.material.opacity < 0.000001) NotVisibleDome.material.opacity = 0.000001;
	//console.log(NotVisibleDome.material.opacity, VisibleDome.material.opacity);
