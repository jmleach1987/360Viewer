...
var camera, scene, renderer, raycaster, plight, ImageDome, dirLight, controls, effect;
...
function init() {
	...
	ImageDome = new THREE.Mesh( geometry, material );
	scene.add(ImageDome);
...
function HideShowImage(event) {
	...
	*/
	...
	//ImageDome.visible = !ImageDome.visible;
}
...
function PositionRig() {
	...
	ImageDome.material.map = THREE.ImageUtils.loadTexture( 'src/Dome_Projects/'+wd.project+'/' + lowRes + wd.CamPos[chosenPOV].name+'.jpg' );
...
function update() {
	...
	if (isAccel) { 
		...
		// it does look at a point on the ImageDome
